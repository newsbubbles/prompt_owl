...
- **[`skills/prowl-script/SKILL.md`](skills/prowl-script/SKILL.md)** covers writing `.prowl`. The
  difference between `{name(8, 0.0)}` and `{name}` is the whole language, and getting it wrong is
  the most common failure.
- **[`skills/prompt-owl/SKILL.md`](skills/prompt-owl/SKILL.md)** covers running the studio, its
  HTTP API, and measuring a stack across models and inputs.

## Layout

```
prowl/lib/          the interpreter: prowl.py (fill, types, stops), stack.py, vllm.py
prowl/tools/        {@tool(...)} implementations, each a folder with tool.py
